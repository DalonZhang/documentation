...
.. tip::
   If the IoT box is connected to an external monitor, you can also scan the first QR code
   displayed on the screen to connect to the network, then scan the second QR code to access the
   IoT box's :ref:`homepage <iot/iot-box/homepage>`. On the homepage, click :guilabel:`Configure` in
   the :guilabel:`Internet Status` section, select the relevant Wi-Fi network, enter the password,
...
- connecting the IoT box to an :ref:`external monitor <devices/screen>`: the IP address is displayed
  on the screen.
